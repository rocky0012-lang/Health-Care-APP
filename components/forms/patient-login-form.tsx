...
export const PatientLoginForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof loginSchema>) => {
    ...
    try {
      const record = await getPatientLoginRecord(values)
      ...
      setCurrentPatientUserId(record.userId)
      setStoredPatientName(record.userId, record.name || "Patient")

      if (record.hasProfile) {
        await activatePatientSession(record.userId)
        clearPendingPatientUserId()
      ...
      setPendingPatientUserId(record.userId)
      await beginPendingPatientSession(record.userId)
      router.push(`/patients/${record.userId}/register`)
      router.refresh()
    } catch (error: any) {
            ...
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="max-w-md text-balance text-base text-muted-foreground md:text-lg">
                  Log in to your NetCare patient account
                </p>
              </div>
              <CustomFormField
                id="email"
